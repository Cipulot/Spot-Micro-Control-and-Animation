...
class SpotAnim:
    ...
    def display_rotate(self, x_spot, y_spot, z_spot, theta_spot, thetax, thetaz, xl, yl, zl):
        ...
        for i in range(0, len(xl)):
            ...
            out0 = Spot.new_coordinates(
                Ma, xl[i], yl[i], zl[i], x_spot, y_spot, z_spot)
    ...
    def animate(self, pos, t, thetax, thetaz, angle, center_x, center_y, thetalf, thetarf, thetarr, thetalr, walking_speed, walking_direction, steering, stance):
        ...
        "Floor Display"""
        line = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [
                                       theta_spot[0], theta_spot[1], 0, 0, 0, 0], thetax, thetaz, SpotAnim.xFloor, SpotAnim.yFloor, SpotAnim.zFloor)
        pygame.draw.polygon(screen, SpotAnim.GREY, line, 0)

        """Floor Grid Display """
        for i in range(0, 11):
            line = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [
                                           theta_spot[0], theta_spot[1], 0, 0, 0, 0], thetax, thetaz, [-500+i*100, -500+i*100], [-500, 500], [0, 0])
            pygame.draw.lines(screen, SpotAnim.DARK_GREY, False, line, 1)
            line = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [
                                           theta_spot[0], theta_spot[1], 0, 0, 0, 0], thetax, thetaz, [-500, 500], [-500+i*100, -500+i*100], [0, 0])
            pygame.draw.lines(screen, SpotAnim.DARK_GREY, False, line, 1)

        """ X,Y,Z frame display"""
        line = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [
                                       0, 0, 0, 0, 0, 0], thetax, thetaz, SpotAnim.xX, SpotAnim.yX, SpotAnim.zX)
        pygame.draw.lines(screen, SpotAnim.RED, False, line, 2)

        line = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [
                                       0, 0, 0, 0, 0, 0], thetax, thetaz, SpotAnim.xY, SpotAnim.yY, SpotAnim.zY)
        pygame.draw.lines(screen, SpotAnim.GREEN, False, line, 2)

        line = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [
                                       0, 0, 0, 0, 0, 0], thetax, thetaz, SpotAnim.xZ, SpotAnim.yZ, SpotAnim.zZ)
        pygame.draw.lines(screen, SpotAnim.BLUE, False, line, 2)

        """ Radius display """
        center_display = True
        if (steering < 2000):
            lineR = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [
                                            0, 0, 0, 0, 0, 0], thetax, thetaz, [center_x, x_spot[0]], [center_y, y_spot[0]], [0, 0])
        else:
            center_x1 = x_spot[0] + (center_x-x_spot[0])/steering*2000
            center_y1 = y_spot[0] + (center_y-y_spot[0])/steering*2000
            lineR = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [
                                            0, 0, 0, 0, 0, 0], thetax, thetaz, [center_x1, x_spot[0]], [center_y1, y_spot[0]], [0, 0])
            center_display = False

        """ Direction display """
        xd = x_spot[0] + walking_speed * \
            cos(theta_spot[2] + walking_direction-pi/2)
        yd = y_spot[0] + walking_speed * \
            sin(theta_spot[2] + walking_direction-pi/2)
        lineD = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [
                                        0, 0, 0, 0, 0, 0], thetax, thetaz, [xd, x_spot[0]], [yd, y_spot[0]], [0, 0])
        ...
        lineCG = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [
                                         0, 0, 0, 0, 0, 0], thetax, thetaz, [CGabs[0], CGabs[0]], [CGabs[1], CGabs[1]], [0, CGabs[2]])

        xleglf = [Spot.xlf, Spot.xlf+leglf[0], Spot.xlf +
                  leglf[1], Spot.xlf+leglf[2], Spot.xlf+pos[0]]
        yleglf = [Spot.ylf, Spot.ylf+leglf[3], Spot.ylf +
                  leglf[4], Spot.ylf+leglf[5], Spot.ylf+pos[1]]
        zleglf = [0, leglf[6], leglf[7], leglf[8], pos[2]]
        linelf = SpotAnim.display_rotate(
            self, x_spot[1]-x_spot[0], y_spot[1]-y_spot[0], z_spot[1]-z_spot[0], theta_spot, thetax, thetaz, xleglf, yleglf, zleglf)

        xlegrf = [Spot.xrf, Spot.xrf+legrf[0], Spot.xrf +
                  legrf[1], Spot.xrf+legrf[2], Spot.xrf+pos[3]]
        ylegrf = [Spot.yrf, Spot.yrf+legrf[3], Spot.yrf +
                  legrf[4], Spot.yrf+legrf[5], Spot.yrf+pos[4]]
        zlegrf = [0, legrf[6], legrf[7], legrf[8], pos[5]]
        linerf = SpotAnim.display_rotate(
            self, x_spot[1]-x_spot[0], y_spot[1]-y_spot[0], z_spot[1]-z_spot[0], theta_spot, thetax, thetaz, xlegrf, ylegrf, zlegrf)

        xlegrr = [Spot.xrr, Spot.xrr+legrr[0], Spot.xrr +
                  legrr[1], Spot.xrr+legrr[2], Spot.xrr+pos[6]]
        ylegrr = [Spot.yrr, Spot.yrr+legrr[3], Spot.yrr +
                  legrr[4], Spot.yrr+legrr[5], Spot.yrr+pos[7]]
        zlegrr = [0, legrr[6], legrr[7], legrr[8], pos[8]]
        linerr = SpotAnim.display_rotate(
            self, x_spot[1]-x_spot[0], y_spot[1]-y_spot[0], z_spot[1]-z_spot[0], theta_spot, thetax, thetaz, xlegrr, ylegrr, zlegrr)

        xleglr = [Spot.xlr, Spot.xlr+leglr[0], Spot.xlr +
                  leglr[1], Spot.xlr+leglr[2], Spot.xlr+pos[9]]
        yleglr = [Spot.ylr, Spot.ylr+leglr[3], Spot.ylr +
                  leglr[4], Spot.ylr+leglr[5], Spot.ylr+pos[10]]
        zleglr = [0, leglr[6], leglr[7], leglr[8], pos[11]]
        linelr = SpotAnim.display_rotate(
            self, x_spot[1]-x_spot[0], y_spot[1]-y_spot[0], z_spot[1]-z_spot[0], theta_spot, thetax, thetaz, xleglr, yleglr, zleglr)
        """ Body frame lines """
        lineb = SpotAnim.display_rotate(self, x_spot[1]-x_spot[0], y_spot[1]-y_spot[0], z_spot[1]-z_spot[0], theta_spot, thetax, thetaz, [
                                        Spot.xlf, Spot.xrf, Spot.xrr, Spot.xlr, Spot.xlf], [Spot.ylf, Spot.yrf, Spot.yrr, Spot.ylr, Spot.ylf], [Spot.zlf, Spot.zrf, Spot.zrr, Spot.zlr, Spot.zlf])
        ...
        linedCG = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [
                                          0, 0, 0, 0, 0, 0], thetax, thetaz, [CGabs[0], dCG[0]], [CGabs[1], dCG[1]], [0, 0])
        ...
        pygame.draw.lines(screen, SpotAnim.BLACK, False, [
                          [angle[0]/pi*180/45*300+300, 0], [angle[0]/pi*180/45*300+300, 50]], 5)
        pygame.draw.lines(screen, SpotAnim.BLACK, False, [
                          [0, angle[1]/pi*180/45*300+300], [50, angle[1]/pi*180/45*300+300]], 5)
