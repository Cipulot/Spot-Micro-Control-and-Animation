...
class Spot:
    ...
    def xyz_rotation_matrix(self, thetax, thetay, thetaz, inverse):
        if (inverse == True):
            ...
            t6 = cos(thetax)
            t7 = sin(thetax)
            M = [t2*t4, t3*t6+t4*t5*t7, t3*t7-t4*t5*t6, -t2*t3,
                 t4*t6-t3*t5*t7, t4*t7+t3*t5*t6, t5, -t2*t7, t2*t6]
        else:
            # Rz*Ry*Rx
            t2 = cos(thetaz)
            t3 = sin(thetax)
            ...
            t6 = sin(thetay)
            t7 = cos(thetay)
            M = [t2*t7, t4*t7, -t6, -t4*t5+t2*t3*t6, t2*t5+t3*t4 *
                 t6, t3*t7, t3*t4+t2*t5*t6, -t2*t3+t4*t5*t6, t5*t7]
        return M
    ...
    def IK(self, L0, L1, L2, d, x, y, z, side):  # Inverse Kinematics
        ...
        try:
            ...
            theta2 = -asin(t14*x)+asin(L2*t14 *
                                       sqrt(1/t15*1/t16*t12**2*(-1/4)+1))
    ...
    def FK(self, theta, side):  # Forward Kinematics
        ...
        y_elbow = side*Spot.L0 * \
            cos(theta[0])-(-Spot.d-Spot.L1*cos(theta[1]))*sin(theta[0])
        z_elbow = side*Spot.L0 * \
            sin(theta[0]) + (-Spot.d-Spot.L1*cos(theta[1]))*cos(theta[0])
    ...
    def FK_Weight(self, theta, side):  # Forward Kinematics for calculation of Center of Gravity
        ...
        yCG_Shoulder1 = side*Spot.yCG_Shoulder * \
            cos(theta[0])-Spot.zCG_Shoulder*sin(theta[0])
        zCG_Shoulder1 = side*Spot.yCG_Shoulder * \
            sin(theta[0])+Spot.zCG_Shoulder*cos(theta[0])

        xCG_Leg1 = Spot.xCG_Leg*cos(theta[1]) + Spot.zCG_Leg*sin(theta[1])
        yCG_Leg1 = cos(theta[0])*(Spot.L0*side + side*Spot.yCG_Leg) + sin(theta[0])*(
            Spot.d - Spot.zCG_Leg*cos(theta[1]) + Spot.xCG_Leg*sin(theta[1]))
        zCG_Leg1 = sin(theta[0])*(Spot.L0*side + side*Spot.yCG_Leg) - cos(theta[0])*(
            Spot.d - Spot.zCG_Leg*cos(theta[1]) + Spot.xCG_Leg*sin(theta[1]))

        xCG_Foreleg1 = cos(theta[1])*(Spot.xCG_Foreleg*cos(theta[2]) + Spot.zCG_Foreleg*sin(theta[2])) - sin(
            theta[1])*(Spot.L1 - Spot.zCG_Foreleg*cos(theta[2]) + Spot.xCG_Foreleg*sin(theta[2]))
        yCG_Foreleg1 = cos(theta[0])*(Spot.L0*side + side*Spot.yCG_Foreleg) + sin(theta[0])*(Spot.d + sin(theta[1])*(Spot.xCG_Foreleg*cos(
            theta[2]) + Spot.zCG_Foreleg*sin(theta[2])) + cos(theta[1])*(Spot.L1 - Spot.zCG_Foreleg*cos(theta[2]) + Spot.xCG_Foreleg*sin(theta[2])))
        zCG_Foreleg1 = sin(theta[0])*(Spot.L0*side + side*Spot.yCG_Foreleg) - cos(theta[0])*(Spot.d + sin(theta[1])*(Spot.xCG_Foreleg*cos(
            theta[2]) + Spot.zCG_Foreleg*sin(theta[2])) + cos(theta[1])*(Spot.L1 - Spot.zCG_Foreleg*cos(theta[2]) + Spot.xCG_Foreleg*sin(theta[2])))
    ...
    def start_walk_stop(self, track, x_offset, steering_radius, steering_angle, cw, h_amp, v_amp, height, stepl, t, tstep, theta_spot, x_spot, y_spot, z_spot, step_phase):
        ...
        s = Spot.new_coordinates(
            self, Ms, xc, yc, 0, x_spot[0], y_spot[0], z_spot[0])
        xs = s[0]
        ys = s[1]
        ...
        Ms_updated = Spot.xyz_rotation_matrix(
            self, theta_spot_updated[3], theta_spot_updated[4], theta_spot_updated[2]+theta_spot_updated[5], False)
        # Matrix from absolute space frame to local body frame
        Msi_updated = Spot.xyz_rotation_matrix(
            self, -theta_spot_updated[3], -theta_spot_updated[4], -(theta_spot_updated[2]+theta_spot_updated[5]), True)
        ...
        foot_center = Spot.new_coordinates(
            self, dMs, x_spot[0]-xs, y_spot[0]-ys, 0, xs, ys, 0)
        ...
        for i in range(0, 4):
            alphav[i] = 0
            if (t1 <= Spot.seq[i]):
                # Leg is on the ground (absolute position value unchanged)
                stance[i] = True
            else:
                if (t1 < (Spot.seq[i]+stepl)):

                    # leg is lifted (absolute position value changes)
                    stance[i] = False
                    ...
                    if (step_phase == 'start'):
                        ...
                        alpha[i] = -Spot.seq[i] / \
                            (1-stepl)/2 + \
                            (t2-Spot.seq[i])/stepl/(1-stepl)*Spot.seq[i]
                    if (step_phase == 'stop'):
                        alpha[i] = -1/2 + Spot.seq[i] / \
                            (1-stepl)/2 + \
                            (t2-Spot.seq[i])/stepl*(1-Spot.seq[i]/(1-stepl))
                    if (step_phase == 'walk'):
        ...
        stance_test = np.sum(
            stance)  # if sum = 4 all feet are on the floor --> body balance
        ...
        if (stance_test == 4):
            ...
            if (t1 > (Spot.seq[istart]+stepl)):
                x_abs_comp = x_abs_area[istart]+(
                    x_abs_area[iend]-x_abs_area[istart])*(t1-tstart-stepl)/(0.25-stepl)
                y_abs_comp = y_abs_area[istart]+(
                    y_abs_area[iend]-y_abs_area[istart])*(t1-tstart-stepl)/(0.25-stepl)
            else:
                x_abs_comp = x_abs_area[istart]
                y_abs_comp = y_abs_area[istart]
        else:
        ...
        Msi_comp = Spot.xyz_rotation_matrix(
            self, 0, 0, -theta_spot_updated[2], True)
        # compensation calculation in body center frame
        comp = Spot.new_coordinates(
            self, Msi_comp, x_abs_comp-x_spot[0], y_abs_comp-y_spot[0], 0, 0, 0, 0)
        ...
        if (step_phase == 'start'):
            ...
            if (t1 > (1-ts)):
                kcomp = (1-t1)/ts
                v_amp_t = 0
        Ms_comp = Spot.xyz_rotation_matrix(
            self, 0, 0, theta_spot_updated[2], False)
        # Compensation calculation absoltute space frame
        compt = Spot.new_coordinates(
            self, Ms_comp, (comp[0]-CG[0])*kcomp+x_offset, (comp[1]-CG[1])*kcomp, 0, 0, 0, 0)
        ...
        for i in range(0, 4):
            ...
            frame_corner = Spot.new_coordinates(
                self, Ms_updated, x_frame[i], y_frame[i], z_frame[i], x_framecenter_comp, y_framecenter_comp, z_framecenter_comp)
        ...
        for i in range(0, 4):
            if stance[i] == False:
                ...
                xleg_target = xc + \
                    radii[i]*cos(alphah) - (comp[0]-CG[0]) * \
                    kcomp - x_offset - x_frame[i]
                yleg_target = yc + \
                    radii[i]*sin(alphah) - (comp[1]-CG[1])*kcomp - y_frame[i]

                leg_current = Spot.new_coordinates(
                    self, Msi_comp, x_spot[i+2]-x_framecorner[i], y_spot[i+2]-y_framecorner[i], -z_framecorner[i], 0, 0, 0)
                # interpolate between current position and targe
                if ((Spot.seq[i]+stepl-t1) > tstep):
                    xint[i] = leg_current[0] + \
                        (xleg_target - leg_current[0]) * \
                        (tstep)/(Spot.seq[i]+stepl-t1)
                    yint[i] = leg_current[1] + \
                        (yleg_target - leg_current[1]) * \
                        (tstep)/(Spot.seq[i]+stepl-t1)
                else:
                    xint[i] = xleg_target
                    yint[i] = yleg_target
                ...
                Msi_body = Spot.xyz_rotation_matrix(
                    self, -theta_spot_updated[3], -theta_spot_updated[4], -theta_spot_updated[5], True)
                legs = Spot.new_coordinates(
                    self, Msi_body, xint[i], yint[i], zint[i], 0, 0, 0)
                xleg[i] = legs[0]
                yleg[i] = legs[1]
                zleg[i] = legs[2]
                ...
                foot_abs = Spot.new_coordinates(
                    self, Ms_updated, xleg[i], yleg[i], zleg[i], x_framecorner[i], y_framecorner[i], z_framecorner[i])
                ...
                leg = Spot.new_coordinates(
                    self, Msi_updated, xabs[i]-x_framecorner[i], yabs[i]-y_framecorner[i], zabs[i]-z_framecorner[i], 0, 0, 0)
                xleg[i] = leg[0]
                yleg[i] = leg[1]
                zleg[i] = leg[2]

        x_spot_updated = [foot_center[0], x_framecenter_comp, xabs[0],
                          xabs[1], xabs[2], xabs[3], x_spot[6], x_spot[7], x_spot[8]]
        y_spot_updated = [foot_center[1], y_framecenter_comp, yabs[0],
                          yabs[1], yabs[2], yabs[3], y_spot[6], y_spot[7], y_spot[8]]
        z_spot_updated = [foot_center[2], z_framecenter_comp, zabs[0],
                          zabs[1], zabs[2], zabs[3], z_spot[6], z_spot[7], z_spot[8]]

        pos = [xleg[0], yleg[0], zleg[0], xleg[1], yleg[1], zleg[1], xleg[2], yleg[2], zleg[2],
               xleg[3], yleg[3], zleg[3], theta_spot_updated, x_spot_updated, y_spot_updated, z_spot_updated]
        return pos
    ...
    def moving(self, t, start_frame_pos, end_frame_pos, pos):
        ...
        for i in range(0, 6):
            frame_pos[i] = start_frame_pos[i] + \
                (end_frame_pos[i] - start_frame_pos[i])*t

        theta_spot_updated[3] = frame_pos[0]
        theta_spot_updated[4] = frame_pos[1]
        theta_spot_updated[5] = frame_pos[2]
        # rotation matrix for frame position
        Mf = Spot.xyz_rotation_matrix(
            self, frame_pos[0], frame_pos[1], frame_pos[2], False)
        ...
        frame_center_abs = Spot.new_coordinates(
            self, Ms, frame_pos[3], frame_pos[4], frame_pos[5], x_spot_updated[0], y_spot_updated[0], z_spot_updated[0])
        ...
        for i in range(0, 4):
            frame_corner = Spot.new_coordinates(
                self, Mf, x_frame[i], y_frame[i], z_frame[i], 0, 0, 0)
            frame_corner_abs = Spot.new_coordinates(
                self, Ms, frame_corner[0], frame_corner[1], frame_corner[2], frame_center_abs[0], frame_center_abs[1], frame_center_abs[2])
        ...
        Mi = Spot.xyz_rotation_matrix(self, -theta_spot_updated[3], -theta_spot_updated[4], -(
            theta_spot_updated[2]+theta_spot_updated[5]), True)

        for i in range(0, 4):
            leg = Spot.new_coordinates(self, Mi, x_spot_updated[i+2]-x_frame_corner_abs[i], y_spot_updated[i+2] -
                                       y_frame_corner_abs[i], z_spot_updated[i+2]-z_frame_corner_abs[i], 0, 0, 0)
            xleg[i] = leg[0]
            yleg[i] = leg[1]
            zleg[i] = leg[2]
